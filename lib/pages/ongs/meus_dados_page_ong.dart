...
class MeusDadosPageOng extends StatefulWidget {
  const MeusDadosPageOng({Key? key}) : super(key: key);

  @override
  _MeusDadosPageOngState createState() => _MeusDadosPageOngState();
}

class _MeusDadosPageOngState extends State<MeusDadosPageOng> {
  ...
  void initState() {
    ...
    _userDataStream = _firestore.collection('ong').doc(_user!.uid).snapshots();
  }
          ...
          return Padding(
            ...
            child: ListView(
              children: [
                ...
                ListTile(
                  title: Text('CNPJ:'),
                  subtitle: Text('${userData['cnpj']}'),
                ),
                ListTile(
                  title: Text('Data de criação:'),
                  subtitle: Text(DateFormat('dd/MM/yyyy').format(
                    (userData['dataCriacao'] is DateTime)
                        ? userData['dataCriacao']
                        : DateTime.parse('${userData['dataCriacao']}'),
                  )),
                ),
                ListTile(
                  title: Text('Endereço:'),
...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> _loadUserData() async {
    try {
      var snapshot =
          await _firestore.collection('ong').doc(widget.user.uid).get();
      var userData = snapshot.data();

      if (userData != null) {
        _nameController.text = userData['nome'];
        _cnpjController.text = userData['cnpj'];
        _dataCriacaoController.text = userData['dataCriacao'];
  ...
  Future<void> _updateUserData() async {
    try {
      ...
      await _firestore.collection('ong').doc(widget.user.uid).update({
        'nome': _nameController.text,
        'cnpj': _cnpjController.text,
        'dataCriacao': _dataCriacaoController.text,
